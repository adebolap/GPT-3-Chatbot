...
This repo now includes `scripts/resolve-contxt-conflicts.sh` for the exact conflict set shown in GitHub. Run it with `--dry-run` first, then choose `--side ours` or `--side theirs` depending on which branch currently contains the Contxt version. See `docs/conflict-resolution.md`.
...
## Commit requested later: `0947250c4e713d771073a72d765d9f79279e4945`

The commit `0947250c4e713d771073a72d765d9f79279e4945` is not present in this checkout. `git show --stat --oneline 0947250c4e713d771073a72d765d9f79279e4945` returns `fatal: bad object`, and there is still no remote configured to fetch it from.

To integrate that exact commit in a checkout with a remote, run:

```bash
git fetch origin 0947250c4e713d771073a72d765d9f79279e4945
git cherry-pick 0947250c4e713d771073a72d765d9f79279e4945
```

If the cherry-pick conflicts, use the same Contxt resolution defaults in this document. If that commit belongs to `resolutionsclaude/eloquent-curie-Zd4FF`, fetch the branch first and inspect the commit before resolving:

```bash
git fetch origin resolutionsclaude/eloquent-curie-Zd4FF
git show --stat --oneline 0947250c4e713d771073a72d765d9f79279e4945
git cherry-pick 0947250c4e713d771073a72d765d9f79279e4945
```

Do not attempt to recreate the commit by guessing; request the patch/diff or fetchable remote if the object is unavailable.
