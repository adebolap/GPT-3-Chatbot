...
## If GitHub says only `package.json` is conflicted

Use this resolved root `package.json` content for the Contxt branch:

```json
{
  "name": "contxt-monorepo",
  "private": true,
  "workspaces": [
    "apps/*",
    "packages/*"
  ...
  "scripts": {
    "dev:web": "npm run dev -w @contxt/web",
    "dev:extension": "npm run dev -w @contxt/extension",
    "build:web": "npm run build -w @contxt/web",
    "build:extension": "npm run build -w @contxt/extension",
...
git commit -m "Resolve package.json conflict for Contxt workspace"
git push
```

For the skipped `.gitignore` and `README.md`, keep the Contxt versions from this branch unless GitHub shows new conflict markers after resolving `package.json`.

## One-command helper for the screenshot branch

The screenshot branch is `codex/build-mvp-for-promptrefiner-browser-extension-gu8m6c`. In a normal local clone with GitHub access, run:
